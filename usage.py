...
batch_size = 50
data_size = 500
states_file = 'trained/trained_translation_transformer.pt'
...
model.load_state_dict(torch.load(states_file, map_location=torch.device('cpu')))
input_sentence = ['Will she read the article?']
tokenized_output = [0]
...
try:
    while not is_end_token(tokenized_output[0]):
        ...
        index_max =  torch.tensor([torch.argmax(out_probs)], dtype=int).unsqueeze(0)
        tokenized_output = torch.cat([tokenized_output, index_max], dim=1)
